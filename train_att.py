...
from att_bilstm import ATT_BiLSTMSentiment
import copy
...
def train_epoch_progress(model, train_iter, loss_function, optimizer, text_field, label_field, epoch):
    ...
    for batch in tqdm(train_iter, desc='Train epoch '+str(epoch+1)):
        ...
        #print(batch.text,type(batch.text),np.shape(batch.text))
        sent = batch.text
        sent_len=[]
        shape=np.shape(sent)
        sent_len+=[shape[0]]*shape[1]
        label.data.sub_(1)
        truth_res += list(label.data)
        model.batch_size = len(label.data)
        model.hidden = model.init_hidden()
        # import pdb;pdb.set_trace()
        pred = model.forward(sent.cuda(),sent_len)
...
def load_sst(text_field, label_field, batch_size):
    ...
    train_iter = data.BucketIterator(train,
                batch_size=batch_size, sort_key=lambda x: len(x.text), repeat=False)
...
best_model = model
optimizer = optim.Adam(model.parameters(), lr=0.2*1e-3)
